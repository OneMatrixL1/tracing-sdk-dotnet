...
// result.Hash == "0x1c8a…", result.Response.StatusCode == 200, result.Response.RecordCount == 1
```
...
- **[`examples/Tracing.Sdk.Examples`](examples/Tracing.Sdk.Examples/Program.cs)** — runnable examples, including the query-then-verify flow
...
- **Hashing.** Keccak-256 (the original Keccak, as used by Ethereum — not FIPS-202 SHA3-256, which is what .NET's `SHA3_256` implements), via [BouncyCastle](https://www.bouncycastle.org/)'s `KeccakDigest`. Strings are hashed as UTF-8. Output is a `0x`-prefixed hex string.
